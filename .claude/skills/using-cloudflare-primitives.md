...
## Scaling cron Workers under the subrequest budget

The 1000-subrequest-per-invocation cap is the failure mode that bites every per-row cron Worker (ingest pipelines, batch summarizers, anything looping over docs with `fetch` + D1 + LLM + UPSERT inside). Symptom: the run partially completes — first N rows succeed, the rest are silently truncated — and the row count in your `*_runs` table is less than `MAX_REGS`. **The instinct to raise the cap, retry harder, or run more often is wrong on all three counts.** The fix is to reshape the workload to fit the budget, then chain invocations if the corpus is bigger than one Worker can chew. In order of effectiveness: (1) pre-fetch row state via batched `SELECT … WHERE id IN (?, ?, …)` chunked at ~100 ids — turns N per-doc SELECTs into ~3; (2) batch UPSERTs via `env.DB.batch(stmts.chunk(50))` — turns N UPSERTs into N/50; (3) lower MAX_REGS to leave 50% subrequest headroom for retry storms (raising it is the opposite of the fix); (4) chain invocations via a Workers Queue consumer or a self-re-enqueue pattern (write next offset to KV, follow-up cron exits early if no offset); (5) tier the work into two Workers (`*-fetch` and `*-summarize`) if the corpus is 1000+ docs — each gets its own cap.

Anti-patterns that look productive but aren't: `for (let attempt = 0; attempt < 5; attempt++)` inside the per-row loop (burns budget faster, doesn't address the cap); running the cron every hour instead of twice weekly (24× invocations × same work = same throughput, 24× cost); raising the CPU time limit (wrong limit — subrequest count hits first). Diagnostic: `wrangler tail` shows the run terminating mid-stream with no error log; the `*_runs` row shows `processed + failures < MAX_REGS`. Per-row subrequest math: 1× upstream fetch + 1× D1 SELECT + 1-3× AI Gateway + 1× UPSERT + 0-1× KV write = 4-7 per row. At MAX_REGS=200 you're 800-1400 — over cap. Codified after `pp-ingest` rounds R29 (initial reliability pass), R84 (cost cap), R125 (subrequest cap incident), R165 (chained-run pattern).
